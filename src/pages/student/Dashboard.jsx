      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm p-6 min-h-[300px] flex flex-col justify-between">
          <div className="flex justify-between items-center mb-6">
            <h3 className="text-base sm:text-lg font-black text-slate-900 dark:text-slate-100 uppercase tracking-wider flex items-center"><FileText className="mr-2.5 text-purple-600" size={20}/> Recent Homework</h3>
          </div>
          <div className="space-y-4 flex-1 overflow-y-auto">
            {recentHomework.length > 0 ? recentHomework.map((hw, idx) => (
              <div key={idx} className="p-4 border-l-4 border-purple-600 bg-purple-50 dark:bg-purple-950/20 rounded-r-2xl shadow-xs">
                <h4 className="text-sm font-black text-slate-900 dark:text-slate-100">{hw.title}</h4>
                <p className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-2 line-clamp-1">{hw.description}</p>
                <div className="mt-3 text-xs font-black text-red-600 dark:text-red-400">Due: {hw.dueDate}</div>
              </div>
            )) : (
              <div className="text-center py-8 text-slate-600 dark:text-slate-400 text-sm font-bold italic">No recent homework.</div>
            )}
          </div>
        </div>
