...
import { CreditCard, CheckCircle, Clock, Eye, AlertCircle } from 'lucide-react';
import ExportButtons from '../../components/ExportButtons';

const ParentFeeStatus = () => {
  ...
  useEffect(() => {
    if (selectedChildId) {
      ...
      const childFees = allFees.filter(f => f.studentId?.toString() === selectedChildId.toString());
  ...
  return (
    <div className="space-y-6 text-xs">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 gap-4">
        <div>
          <h2 className="text-2xl font-bold text-gray-800">Children Fee Statements</h2>
          <p className="text-gray-500 text-sm mt-1">Review active tuition and admission balance dues for your children.</p>
        </div>
        <ExportButtons tableId="export-table" filename="Children_Fee_Status" />
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        {filteredFees.length === 0 ? (
          <div className="p-12 text-center text-gray-500 flex flex-col items-center">
            ...
            <p className="text-xs font-semibold">No fee history found for this child.</p>
          </div>
        ) : (
