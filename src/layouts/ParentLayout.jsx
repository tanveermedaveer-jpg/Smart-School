...
import { 
  ...
  CreditCard, Bell, Phone, LogOut, User, Sun, Moon, Menu, X 
} from 'lucide-react';
...
const ParentLayout = () => {
    ...
    const doSync = async () => {
      if (authUser?.schoolId) {
        try {
          ...
          console.error('Error syncing ParentLayout:', err);
        }
      }
      setIsSyncing(false);
    };
  ...
  const menuItems = [
    ...
    { path: '/parent/child-profile', icon: <UserCheck size={20} />, label: 'Child Profile' },
    { path: '/parent/attendance', icon: <UserCheck size={20} />, label: 'Attendance' },
    { path: '/parent/homework', icon: <Book size={20} />, label: 'Homework' },
    { path: '/parent/results', icon: <Award size={20} />, label: 'Child Results' },
    { path: '/parent/timetable', icon: <Book size={20} />, label: 'Timetable' },
    { path: '/parent/fee-status', icon: <CreditCard size={20} />, label: 'Fee Status' },
    { path: '/parent/notices', icon: <Bell size={20} />, label: 'Notices' },
    { path: '/parent/contact', icon: <Phone size={20} />, label: 'Contact School' },
    { path: '/parent/profile', icon: <User size={20} />, label: 'My Profile' },
  ];
    ...
    <div className="flex h-screen bg-gray-100 font-poppins overflow-hidden" data-theme={isDark ? 'dark' : 'light'}>
      ...
      `}>
        <div className="p-6 border-b border-white/10 flex items-center justify-between lg:justify-center">
          <h1 className="text-xl font-bold text-center leading-tight">Parent<br/><span className="text-sm font-light text-greenAccent">Portal</span></h1>
          <button 
            onClick={() => setIsMobileMenuOpen(false)}
        ...
        <header className="bg-white border-b border-gray-200 h-16 flex items-center px-4 sm:px-8 shrink-0 justify-between lg:justify-end sticky top-0 z-10 shadow-sm">
          ...
          <div className="flex items-center space-x-3">
             ...
             <div className="flex items-center space-x-2 sm:space-x-3">
               <div className="w-8 h-8 bg-greenAccent rounded-full flex items-center justify-center text-white font-bold text-sm uppercase overflow-hidden shadow-sm shrink-0">
                 {avatar ? (
                   <img src={avatar} alt="Avatar" className="w-full h-full object-cover" />
                 ) : (
                   authUser?.name?.substring(0,2) || 'P'
                 )}
               </div>
               <span className="text-xs sm:text-sm font-medium text-gray-700 dark:text-slate-200 truncate max-w-[120px] sm:max-w-none">{authUser?.name || 'Parent'}</span>
             </div>
           </div>
        </header>
